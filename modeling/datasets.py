...
class EmbeddingsDataset:
    ...
    def __getitem__(self, idx):
        ...
        return {"item_id": self.item_ids[idx], "embedding": tensor_emb, "embedding_dim": len(tensor_emb)}
    ...
    def get_interactions_by_part(self, start_part: int, end_part: int):
        part_interactions = self.all_interactions.filter((pl.col("part") >= start_part) & (pl.col("part") < end_part))
...
class SequentialDataset:
    ...
    def _create_train_samples(self):
        ...
        for row in history_grouped.iter_rows(named=True):
            ...
            samples.append({"user.ids": [user_id], "item.ids": items[-(self.max_sequence_length + 1) :]})
    ...
    def _create_eval_samples(
        ...
        history_interactions = self._get_interactions_by_part(self.train_parts[0], eval_start_part)
        history_grouped = self._group_by_users(history_interactions)

        label_interactions = self._get_interactions_by_part(eval_start_part, eval_end_part)
    ...
    def _get_interactions_by_part(self, start_part: int, end_part: int):
        return self.all_interactions.filter((pl.col("part") >= start_part) & (pl.col("part") < end_part))

    def _group_by_users(self, interactions: pl.DataFrame) -> pl.DataFrame:
        grouped = (
            interactions.sort(by="original_order")
            .select(["original_order", "user_id", "item_id"])
            .group_by("user_id", maintain_order=True)
            .agg(pl.col("item_id").sort_by("original_order").alias("item_ids"), pl.col("original_order").sort())
            .sort("user_id")
        )
        return grouped


class FinetuneDataset(SequentialDataset):
    def __init__(
        self,
        all_interactions_path,
        all_embeddings_path,
