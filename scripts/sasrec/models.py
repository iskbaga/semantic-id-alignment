...
class SasRecModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        all_sample_embeddings = self._encoder(embeddings=embeddings, lengths=all_sample_lengths,
                                              max_seqlen=max_seqlen)  # (total_batch_items, embedding_dim)
        ...
        if self.training:
            ...
            positive_items, _ = create_masked_tensor(
                data=all_positive_sample_events,
                lengths=all_positive_sample_lengths,
                padding_value=-1
            )  # (batch_size, num_pos)

            _, topk_indices = torch.topk(
                all_scores, k=20, dim=-1, largest=True, sorted=True
            )  # (batch_size, max_k)
            ...
            for k in [1, 5, 10, 20]:
                metrics[f'recall@{k}'] = recall(
                    all_hits=all_hits,
                    positive_lengths=all_positive_sample_lengths,
                    k=k
                )
