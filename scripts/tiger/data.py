...
class TigerEvalDataset:
    ...
    def __getitem__(self, index):
        ...
        assert (
                item_semantic_ids != -1).all(), f'Missing mappings detected in item! Invalid positions: {(item_semantic_ids == -1).sum()}'

        label_ids = sample['label.ids']
        label_semantic_ids = self.mapping_array[label_ids].flatten()

        assert (
                label_semantic_ids != -1).all(), f'Missing mappings detected in label! Invalid positions: {(label_semantic_ids == -1).sum()}'
...
def tiger_preprocess(batch, num_codebooks, codebook_size):
    ...
    new_flatten_input_semantic_ids = torch.zeros(
        total_length,
        dtype=input_semantic_ids.dtype,
        device=input_semantic_ids.device
    )
