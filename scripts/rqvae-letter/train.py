...
from codebook_utils import fix_dead_codebooks, codebook_initialize
...
def generate_constants(cfg: DictConfig):
    split_name = f"{cfg.dataset.train_parts[0]}-{cfg.dataset.train_parts[1]}TR_" \
                 f"{cfg.dataset.val_parts[0]}-{cfg.dataset.val_parts[1]}V_" \
                 f"{cfg.dataset.test_parts[0]}-{cfg.dataset.test_parts[1]}T"

    results_path = os.path.join(cfg.paths.results_dir, split_name, 'rqvae-letter')
    ...
    experiment_name = f'letter_{cfg.dataset.name}_{split_name}'

    sasrec_results_path = os.path.join(cfg.paths.results_dir, split_name, 'sasrec')

    return {
        'INTERACTIONS_PATH': interactions_path,
        'EMBEDDINGS_PATH': embeddings_path,
        'EXPERIMENT_NAME': experiment_name,
        'SASREC_EMBEDDINGS_PATH': os.path.join(sasrec_results_path, 'embeddings.pt'),
        'INFERENCE_PATH': os.path.join(results_path, 'all_clusters.json'),
        'ALL_MAPPING_PATH': os.path.join(results_path, 'all_clusters_colisionless.json'),
        'OUTPUT_TRAIN_MAPPING_PATH': os.path.join(
            results_path,
...
def train_letter(cfg: DictConfig):
    ...
    cf_embeddings = torch.load(consts['SASREC_EMBEDDINGS_PATH'], map_location='cpu')
    ...
    logger.info(f'Inference metrics: ' + ', '.join(f'{k} {v:.4f}' for k, v in inference_metrics_prefixed.items()))

    with open(consts['INFERENCE_PATH'], 'r') as f:
    ...
    for _, items in sem_2_ids.items():
